...
Ameliorations v2:
- Parametre ROS `audio_enabled` (bool, defaut True) pour desactiver le son
  pendant les tests sans modifier le code.
- Topic `/smartnav/feedback_log` (String) qui publie le type d'alerte
  declenchee avec timestamp — utile pour analyser les sessions de simulation.
"""
...
class CanneFeedbackNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # Parametre pour desactiver le son (pratique lors des tests)
        ...
        # Topic de log des alertes avec timestamp pour analyse post-session
        self._feedback_log_pub = self.create_publisher(String, '/smartnav/feedback_log', 10)

        self._current_vibration: int = 0
        self._vibration_timer = self.create_timer(0.1, self._vibration_heartbeat)

        audio_status = 'ACTIF' if self.get_parameter('audio_enabled').value else 'DESACTIVE'
        self.get_logger().info(
            f'canne_feedback_node demarre — son {audio_status} et vibration actifs.'
        )
    ...
    def _apply_effective_alert(self) -> None:
        ...
        audio_enabled = bool(self.get_parameter('audio_enabled').value)
        ...
        if level == 2:
            ...
            self._log_feedback('DANGER', level)
            ...
            self._log_feedback('ATTENTION', level)
            ...
            if prev > 0:
                ...
                self._log_feedback('LIBRE', level)
    ...
    def _log_feedback(self, alert_type: str, level: int) -> None:
        """Publie un log horodate sur /smartnav/feedback_log."""
        timestamp = datetime.now().strftime('%H:%M:%S.%f')[:-3]
        log_entry = f'[{timestamp}] {alert_type} (level={level}) | ' \
                    f'lidar={self._lidar_alert_level} crossing={self._crossing_alert_level}'
        log_msg = String()
        log_msg.data = log_entry
        self._feedback_log_pub.publish(log_msg)
